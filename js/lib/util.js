...
function canMove(id, move, emptyId) { // id:原index, move:x,y方向的+-1
...
function checkSuccess(databus) {
  if (databus.emptyIndex !== 15) {
    return false;
  } else {
    var data = databus.squares;
    for (var i = 0; i < 15; i++) {
      if (data[i].num !== i + 1) {
        return false;
      }
